 ...
 * ***************************************************
 */
function zColorNoWebGL(poly1,poly2,poly3,poly4){
    /*

    Calculo de cor com webGl desactivado

    Esta funcao retorna um valor adimensional, com maximo de 0.5, 
    proporcional ao heat map dos tempos de ignicao

    Variaveis globais:
    - ROWS
    - COLS
    - heatMap: mapa de tempos de ignicao a partir do qual o gradiente 
                da superficie do gráfico vai ser plotado
    - maxHeatMap: valor maximo do array heatMap
    */

    //a funcao e chamada na linha 408!!

    var row = Math.floor((ROWS - 1)*poly1.ly + (ROWS - 1)/2);
    var col = Math.floor(( COLS - 1)*poly1.lx + ( COLS - 1)/2);
    var idx1 = col + ( COLS)*row;

    row = Math.floor((ROWS - 1)*poly2.ly + (ROWS - 1)/2);
    col = Math.floor(( COLS - 1)*poly2.lx + ( COLS - 1)/2);
    var idx2 = col + ( COLS)*row;

    row = Math.floor((ROWS - 1)*poly3.ly + (ROWS - 1)/2);
    col = Math.floor(( COLS - 1)*poly3.lx + ( COLS - 1)/2);
    var idx3 = col + ( COLS)*row;

    row = Math.floor((ROWS - 1)*poly4.ly + (ROWS - 1)/2);
    col = Math.floor(( COLS - 1)*poly4.lx + ( COLS - 1)/2);
    var idx4 = col + ( COLS)*row;

    var colourValue = heatMap[idx1] /plotTime/2;//maxHeatMap/2;

    return colourValue;
}

function zColorWebGl(poly){
    /*

    Calculo de cor com webGl activado
    ...
    */
    ...
    var row = Math.floor((ROWS - 1)*poly.ly + (ROWS - 1)/2);
        ...
        else // Render not quite so shiny non-GL surface-plot.
        {
            ...
            for (i = 0; i < polygons.length; i++) {
                ...
                if (polygon.isAnAxis()) {
                    ...
                    var rgbColour = this.colourGradientObject.getColour(zColorNoWebGL(p1,p2, p3, p4));
    ...
    this.initSurfaceBuffers = function(){
        ...
        for (i = 0; i < this.numXPoints - 1; i++) {
            for (j = 0; j < this.numYPoints - 1; j++) {
                ...
                // Surface colours.
                var rgb1 = this.colourGradientObject.getColour(zColorWebGl(rawP1));
                var rgb2 = this.colourGradientObject.getColour(zColorWebGl(rawP2));
                var rgb3 = this.colourGradientObject.getColour(zColorWebGl(rawP3));
                var rgb4 = this.colourGradientObject.getColour(zColorWebGl(rawP4));
